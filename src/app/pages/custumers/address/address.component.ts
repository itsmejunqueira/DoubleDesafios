...
export class AddressComponent implements OnInit {
  ...
  ngOnInit(): void {
   this.sort();
   this.selectedBodyDataBkp = JSON.parse(JSON.stringify(this.selectedBodyData));
  }
  ...
  public createAdress() {
    ...
    while (cepArray.length <3) {
      ...
      if (!cepArray.includes(cep))
          cepArray.push(cep)
    }
    this.selectedBodyData = this.selectedBodyData.concat([{
      cep: cepArray[0],
      street: 'R. Antônio Gomes Ferreira',
      number: '358',
      city: 'Campinas (SP)',
  ...
  public sort(): void {
   this.isAsc = !this.isAsc;
    if (this.isAsc)
     ...
     this.selectedBodyData.sort((a, b) => b.street.localeCompare(a.street))
  }

  public searchByString(): void {
    if (this.search)
     this.selectedBodyData = this.selectedBodyDataBkp.filter((x)=>
  ...
  compareString(str:string,str2:string){
    return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase().includes(
      str2.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase()
    )
  }

  clearFilter(){
    this.selectedBodyData = JSON.parse(JSON.stringify(this.selectedBodyDataBkp));
  }
}
